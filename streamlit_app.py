...
st.set_page_config(page_title="Information Field Theory – Wiener Filter", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📡 Information Field Theory – Wiener Filter</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Macro‑conditioned Wiener filter on ETF correlation graph | Posterior uncertainty as signal</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 IFT Wiener")
...
st.sidebar.markdown(f"**Macro vars:** {', '.join(config.MACRO_VARS)}")
st.sidebar.markdown(f"**Graph K‑NN:** {config.GRAPH_KNN} | **Signal/noise:** {config.SIGNAL_NOISE_RATIO}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'ift_wiener_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Posterior variance: {etf['ift_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.6f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Uncertainty"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Uncertainty", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📡 Top ETFs by IFT Posterior Variance (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Information field theory** treats the ETF return field as a stochastic field on the correlation graph.
        - Macro variables (VIX, DXY, yields) enter as external source fields (prior mean).
        - The Wiener filter computes the posterior mean and variance given noisy observations.
        - **Score = posterior variance** – high values indicate macro conditions conflict with graph‑implied relationships → regime uncertainty → potential alpha.
        - Low variance → market is coherent and well‑explained by macro and graph structure.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the posterior variance scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Information Field Theory | Wiener filter with macro priors")
